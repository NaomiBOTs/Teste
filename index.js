...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hello, Sejam Bem Vindo (a) @${num.split('@')[0]}\n Espero que voc� goste do (a) *${mdata.subject}* Caso queira ver meus comandos... Digite *.menu*`
				...
				teks = `Sayonara, Ja Foi Tarde em brother  @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: ' Atualmente em processo, Caso nao obtenha... Tente Novamente!!! ',
				...
				only: {
					...
					ownerB: '[] Este comando s� pode ser usado pelo   ',
					admin: '[] Vai se Fuder Membro Comum Este comando s� pode ser usado por algum administrador!! ',
					Badmin: '[] Eu nem sou administrador Fdp, voc� acha que eu sou Deus? Me da admin a� e tente esse comando novamente !!! ',
                                        daftarB: `──「 DAFTAR 」──\nOla Amig�o !\nPara voc� usar nossos comando, voc� precisa se registrar... \n\nComando para Registro : ${prefix}daftar (Seu nome)\nExemplo : ${prefix}daftar Rodrigo`,
				}
			}
			...
			const ownerNumber = ["557599056928@s.whatsapp.net"] // replace this with your number
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`O @${mentioned[0].split('@')[0]} Virou membro comumkkjjkkkj!`, mentioned, true)
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Vixi, novo adm ai galera @${mentioned[0].split('@')[0]} Agora vai banir geral!`, mentioned, true)
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui est� o n�mero do meu dono, mana, salve mano, salve depois',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde est� o nome de usu�rio, mana? ', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
						...
						reply('nome de usu�rio n�o � valid')
					}
					break
				case 'snowwrite':
		...
		case 'spamcall':
            ...
            break
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Eu disse para voc� escrever, irm�?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual � o c�digo da linguagem, tio?', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Pede a sua mae para ler isso tudo, aquela fela da puta')
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Humor Rapaziada.'})
					break
				case 'memeindo':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 HELLO RAPAZIADA 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 COLEEE MEU BROTHER 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'chord':
					if (args.length < 1) return reply('onde est� o t�tulo da m�sica mana')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Cad� a letra da m�sica, mana?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Coloca o n�mero da pessoa que voc� deseja adicionar, de prefer�ncia o de sua m�e.')
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Banidokkkkkkkkkkjjjkk, Ademir ta bravo :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Banidokkkkkkkkkkjjjkk, Ademir ta bravo : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('voc� se registrou ')
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`*Parabens, voce acabou de se registrar!!!*\nAgora pode usar o bot tranquilamente. SN: TM08GK8PPHBSJDH10J\`\`\`\n\n\`\`\`Pada ${date} ${time}\`\`\`\n\`\`\`[Nama]: ${jeneng}\`\`\`\n\`\`\`[Nomor]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Umur]: ${umure}\`\`\`\n\`\`\`Untuk menggunakan bot\`\`\`\n\`\`\`silahkan\`\`\`\n\`\`\`kirim ${prefix}help\`\`\`\n\`\`\`\nTotal Pengguna ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW est� DESLIGADO* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
